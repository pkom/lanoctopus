...
exports.postLogin = (req, res, next) => {
  ...
  if (!validator.isAlphanumeric(req.body.name)) validationErrors.push({ msg: 'Please enter a valid user name.' });
  ...
  // req.body.username = validator.normalizeEmail(req.body.email, { gmail_remove_dots: false });
  ...
  passport.authenticate('ldapauth', async (err, user, info) => {
    ...
    req.logIn(user, (err) => {
      if (err) { return next(err); }
      req.flash('success', { msg: 'Has accedido al sistema.' });
...
exports.postSignup = (req, res, next) => {
  ...
  if (!validator.isLength(req.body.password, { min: 1 })) validationErrors.push({ msg: 'Password must be at least 1 characters long' });
...
exports.getVerifyEmail = (req, res, next) => {
  ...
  const sendVerifyEmail = (token) => {
    ...
    const mailOptions = {
      to: req.user.email,
      from: 'LanOctopus Server <francisco.mora.sanchez@gmail.com>',
      subject: 'Please verify your email address on LanOctopus Server',
      text: `Thank you for registering your email address with LanOctopus Server.\n\n
